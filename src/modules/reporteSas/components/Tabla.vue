      ...
      >
        ...
        <template #body="{ data }">
          ...
          <span
            ...
            :title="String(data[column.field] ?? '')"
          >
            {{ data[column.field] == null || data[column.field] == 0 ? '' : data[column.field] }}
          </span>
        </template>
      </Column>
    </DataTable>
...
import { createReporteSasFilters, reporteSasColumns, reporteSasRowsOptions, reporteSasTableStyle } from './columns'
...
const dt = ref(null)
const cols = ref(reporteSasColumns)
const first = ref(0)
...
const exportarExcel = () => {
  ...
  exportToExcel({
    ...
    columnTypes: { },
    valueTransformers: {
      cantidadMaterial: (value) => value === 0 ? null : value
    },
//    groupField: null
  })
}
