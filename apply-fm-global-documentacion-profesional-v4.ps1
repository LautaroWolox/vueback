...
$functionalBeforeNormalized = [regex]::Replace($functionalBefore, '\s+', '')
...
function Get-SectionMetadata([string]$name) {
  switch -Regex ($name) {
    ...
    '^fm-menu-tuning\.css$' {
      return New-Metadata 'Menú principal - ajustes visuales' 'GLOBAL CRÍTICO' 'Cabecera, menú principal, submenús y sector de usuario.' 'Apariencia del menú, perfil, cierre de sesión, estados activos, estado al pasar el cursor, bordes, sombras y franja decorativa.' 'No utilizar este bloque para ajustar contenido interno de una pantalla.'
    }
    '^fm-menu-spacing\.css$' {
...
$functionalAfterNormalized = [regex]::Replace($functionalAfter, '\s+', '')
if ($functionalAfterNormalized -cne $functionalBeforeNormalized) {
  throw 'SEGURIDAD: se detectó una modificación funcional fuera de comentarios. No se escribió el archivo.'
}
...
Write-Host 'OK: los encabezados explican componente, alcance y mantenimiento.' -ForegroundColor Green
